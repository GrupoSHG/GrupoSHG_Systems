...
(function() {
  ...
  function CardHEA({ data, loading, error }) {
    if (loading) return <div style={S.loader}>⏳ Cargando HEA...</div>;
    if (error)   return <div style={{...S.loader, color:'#c0392b'}}>❌ {error}</div>;
    if (!data)   return <div style={S.loader}>Sin datos</div>;
    return (
      <div style={S.body}>
        <div style={{...S.kpiMain, background:'#5b2c6f'}}>
          <div style={S.kpiEtq}>Ventas MTD</div>
          <div style={S.kpiMonto}>{fmtCLP(data.ventasMTD)}</div>
          <div style={S.kpiSub}>{data.cantidadOTs} OTs facturables este mes</div>
        </div>
        <div style={S.secTitle}>Detalle</div>
        <div style={S.row}>
          <span style={S.lbl}>Cantidad de OTs</span>
          <span style={{...S.val, color:'#5b2c6f'}}>{data.cantidadOTs}</span>
        </div>
        <div style={S.row}>
          <span style={S.lbl}>Ticket promedio</span>
          <span style={{...S.val, color:'#5b2c6f'}}>{fmtCLP(data.cantidadOTs > 0 ? data.ventasMTD / data.cantidadOTs : 0)}</span>
        </div>
      </div>
    );
  }
